    ...
    if rms > THRESHOLD:
        ...
        if gap >= MIN_GAP:
            if gap <= MAX_GAP and last_clap_time > 0:
                ...
                print(f"[E.V.] Double clap detected! Firing launch script. Shutting down.", flush=True)
                triggered = True
                last_clap_time = 0.0
                subprocess.Popen(["powershell", "-ExecutionPolicy", "Bypass", "-File", SCRIPT_PATH])
            else:
                # First clap
                print(f"[E.V.] First clap detected (rms={rms:.3f})", flush=True)
                last_clap_time = now
...
with sd.InputStream(
    ...
    print("[E.V.] Listening for double clap...", flush=True)
    while not triggered:
        time.sleep(0.1)
    print("[E.V.] Trigger fired — stopped listening.", flush=True)
